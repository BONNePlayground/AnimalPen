...
public class AquariumRenderer implements BlockEntityRenderer<AquariumTileEntity, AnimalPenRenderState>
{
    ...
    @Override
    public void extractRenderState(AquariumTileEntity blockEntity,
        ...
        @Nullable ModelFeatureRenderer.CrumblingOverlay crumblingOverlay)
    {
        ...
        if (animal != null)
        {
            ...
            try (ProblemReporter.ScopedCollector reported = new ProblemReporter.ScopedCollector(animal.problemPath(),
                AnimalPen.LOGGER))
            {
                TagValueOutput tagOutput = TagValueOutput.createWithContext(reported, animal.registryAccess());
                animal.save(tagOutput);
                ValueInput tagInput = TagValueInput.create(reported, animal.registryAccess(), tagOutput.buildResult());

                // Summon entity per death on client and remove it once render state is created.
                blockEntity.getDeathTicker().forEach(tick -> EntityType.create(animal.getType(),
                    tagInput,
                    animal.level(),
                    EntitySpawnReason.TRIGGERED).ifPresent(entity ->
                {
                    ...
                    EntityRenderState entityRenderState = this.entityRenderer.extractEntity(death, partialTick);
    ...
    {
        return AnimalPen.config().isGrowWaterAnimals() || BlockEntityRenderer.super.shouldRender(blockEntity, vec3);
    }
